...
class SRG:
    def __init__(self, v, k, l, u):
        ...
        self._ri = 0
        self._encoded = np.zeros(v - 1, dtype=np.uint8)
    ...
    def pivot_vector(self):
        vec = np.zeros(self._ri, dtype=np.uint8)
        ri = self._ri - 1
    ...
    def to_matrix(self):
        ...
        i = self._ri
        mat = np.zeros((self.v, self.v), dtype=np.uint8)
        while i > 0:
            # decoding
            enc[i:] >>= 1
            i -= 1
    ...
    def _decode(cls, enc, R, C, i):
        ...
        mat = np.zeros((R, C), dtype=np.uint8)
        while i > 0:
            # decoding
            enc[i:] >>= 1
            i -= 1
    ...
    def question(self, include_k=False):
        ...
        inner_prod_required = np.array([self.l if m_ri[r] == 1 else self.u for r in range(ri)], dtype=np.uint8)
        ...
        if include_k:
            R, C = m_right.shape
            constrain_k_vec = np.ones(C, dtype=np.uint8)
